...
final pokemonDetailStateProvider = StateNotifierProvider.autoDispose
    .family<PokemonDetailState, AsyncValue<PokemonDetail>, int>(
        (ref, pokemonId) {
...
class PokemonDetailState extends StateNotifier<AsyncValue<PokemonDetail>> {
  ...
  PokemonDetailState(
      {required this.pokemonDetailUseCase, required this.pokemonId})
      : super(const AsyncValue.loading());

  Future<void> fetchPokemonDetailByID() async {
    try {
      final pokemon = await pokemonDetailUseCase.call(
        PokemonDetailUseCaseParam(pokemonId: pokemonId),
      );
